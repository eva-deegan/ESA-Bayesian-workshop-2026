...
jags.data = list(Spp = data$Spp, Surv = data$Surv, D = as.matrix(data[,3:5]), 
                 k = rowSums(data[,3:5]), alpha = c(1,1,1))

# Initialize model with jags.model(), for 3 chain.
jm = jags.model("model_hurricane.R", data = jags.data, n.chains = 3)

# Update model with coda.samples() and monitor quantities of interest
coda = coda.samples(jm, variable.names = c("p","p.diff","deviance"), 
                    n.iter = 10000)

# Evaluate burn-in, convergence, and mixing:
MCMCtrace(coda, iter=10000, filename = "Hurricane_plots.pdf")
...
MCMCplot(coda,params="p.diff",horiz=FALSE)
MCMCplot(coda,params="p.diff",horiz=FALSE, 
