...
# non-log scale):
tau.log.LUE = 1/(2*(lm1_coef[2,2])^2)
...
jm1 <- jags.model("02_Group_Problems/03_light_response/proj03_mod1.JAGS",
                  ...
                  n.chains = 3)
niter <- 20000
params <- c("deviance", "theta", "mu.theta","sig.log","R2",
...
# Use ggplot and put predicted on the y-axis
photo$An.rep <- out.rep1$mean
photo$An.lower <- out.rep1$`2.5%`
photo$An.upper <- out.rep1$`97.5%`

photo |> 
  ggplot(aes(x = An,
             y = An.rep)) +
  geom_errorbar(aes(ymin = An.lower,
                    ymax = An.upper)) +
  geom_point()

# get R2 from classical regression of obs vs pred:
lm2_sum <- lm(An.rep ~ An, data = photo) |> 
  summary()
...
params <- c("deviance", "theta", "E","S","sig","An.rep","R2",
...
# Observed vs predicted
# Use ggplot and put predicted on the y-axis
photo$An.rep <- out.rep2$mean
photo$An.lower <- out.rep2$`2.5%`
photo$An.upper <- out.rep2$`97.5%`
...
# get R2 from classical regression of obs vs pred:
lm3_sum <- lm(An.rep ~ An, data = photo) |> 
  summary()
lm3_sum$adj.r.squared
